...
class EpochBatchIterator(EpochBatchIterating):
    ...
    Args:
        ...
        max_iter_len (int, optional): if positive, the max number of batch generated for each epoch
    """

    def __init__(
        self, dataset, collate_fn, batch_sampler, seed=1, num_shards=1, shard_id=0,
        num_workers=0, epoch=1, buffer_size=0, max_iter_len=0,
    ):
        ...
        self.max_iter_len = max(max_iter_len, 0)
    ...
    def __len__(self):
        if 0 < self.max_iter_len < len(self.frozen_batches):
            return int(math.ceil(self.max_iter_len / float(self.num_shards)))
        else:
    ...
    def _get_iterator_for_epoch(self, epoch, shuffle, fix_batches_to_gpus=False, offset=0):
        ...
        if self._supports_prefetch:
            ...
            if 0 < self.max_iter_len < len(self.frozen_batches):
                batches = batches[:self.max_iter_len]
